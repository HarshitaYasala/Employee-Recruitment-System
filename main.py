...
Employee_Recruitment_System_bucket = "employee-recruitment-bucket"
Employee_Recruitment_System_email_source = 'nithishkumarvunnam@gmail.com'
s3_client_Employee_Recruitment_System = boto3.client('s3', aws_access_key_id="AKIgtfkjhbkh7fg", aws_secret_access_key="sQBcR7xA8cO4VxDjZGdEU3E0jjdHR0heshfhgi")
ses_client_Employee_Recruitment_System = boto3.client('ses', aws_access_key_id="AKIgtfkjhbkh7fg", aws_secret_access_key="sQBcR7xA8cO4VxDjZGdEU3E0jjdHR0heshfhgi", region_name='us-east-1')

app = Flask(__name__)
app.secret_key = 'kutty'
conn = pymysql.connect(host="localhost", user="root", password="Aishwarya@123", db="jobRecruitment")
cursor = conn.cursor()
...
def add_categories_action():
    ...
    picture.save(path)
    s3_client_Employee_Recruitment_System.upload_file(path, Employee_Recruitment_System_bucket, picture.filename)
...
def view_post_job_recruitments():
    cursor.execute("select * from job_recruitments where recruiter_id = '"+str(session['recruiter_id'])+"'")
...
def job_seeker_login1():
    ...
    if count > 0:
        ...
        emails = ses_client_Employee_Recruitment_System.list_identities(
            IdentityType='EmailAddress'
        )
        if email in emails['Identities']:
            email_msg = 'You' + '' + ' Have Successfully Logged'
            ses_client_Employee_Recruitment_System.send_email(Source=Employee_Recruitment_System_email_source, Destination={'ToAddresses': [email]}, Message={'Subject': {'Data': email_msg, 'Charset': 'utf-8'}, 'Body': {'Html': {'Data': email_msg, 'Charset': 'utf-8'}}})
...
def job_seeker_registration_action():
    ...
    resume.save(path)
    s3_client_Employee_Recruitment_System.upload_file(path, Employee_Recruitment_System_bucket, resume_name)
    ...
    conn.commit()
    ses_client_Employee_Recruitment_System.verify_email_address(
        EmailAddress=email
    )
    return render_template("msg.html", message="Job Seeker Registered successfully. Click on the link to verify your mail, that has been sent your mail")
...
def add_certifications_action():
    ...
    path = APP_ROOT + "/" + certification.filename
    certification.save(path)
    s3_client_Employee_Recruitment_System.upload_file(path, Employee_Recruitment_System_bucket, certification.filename)
    cursor.execute("insert into certifications(job_seeker_id,certification_title,certification) values('"+str(job_seeker_id)+"','"+str(certification_title)+"','"+str(certification.filename)+"')")
    conn.commit()
...
def search_job_recruitment():
    ...
    cursor.execute("SELECT DISTINCT(name) FROM recruiters")
    name = cursor.fetchall()
    return render_template("search_job_recruitment.html",job_recruitments=job_recruitments,job_title=job_title, get_recruiter_by_recruiter_id=get_recruiter_by_recruiter_id, name=name)


def get_recruiter_by_recruiter_id(recruiter_id):
    cursor.execute("select * from recruiters where recruiter_id = '"+str(recruiter_id)+"'")
    recruiters = cursor.fetchall()
    print(recruiters)
    return recruiters[0]
...
def get_job_recruitments():
    ...
    conn.commit()
    job_recruitments =cursor.fetchall()
    return render_template("get_job_recruitments.html",job_recruitments=job_recruitments, get_recruiter_by_recruiter_id=get_recruiter_by_recruiter_id)
...
def accept_application():
    ...
    conn.commit()
    cursor.execute("select * from job_seekers where job_seeker_id in(select job_seeker_id from job_applications where job_application_id='"+str(job_application_id)+"')")
    job_seeker = cursor.fetchall()
    email = job_seeker[0][3]
    name = job_seeker[0][1]
    emails = ses_client_Employee_Recruitment_System.list_identities(
        IdentityType='EmailAddress'
    )
    if email in emails['Identities']:
        msg = 'Hello '+name+' Your Job Application has been Accepted'
        ses_client_Employee_Recruitment_System.send_email(Source=Employee_Recruitment_System_email_source,
                                                          Destination={'ToAddresses': [email]},
                                                          Message={'Subject': {'Data': msg, 'Charset': 'utf-8'},
                                                                   'Body': {'Html': {'Data': msg,
                                                                                     'Charset': 'utf-8'}}})
...
def reject_application():
    ...
    cursor.execute(
        "select * from job_seekers where job_seeker_id in(select job_seeker_id from job_applications where job_application_id='" + str(
            job_application_id) + "')")
    job_seeker = cursor.fetchall()
    email = job_seeker[0][3]
    name = job_seeker[0][1]
    ...
    if email in emails['Identities']:
        msg = 'Hello ' + name + ' Your Job Application has been Rejected'
        ses_client_Employee_Recruitment_System.send_email(Source=Employee_Recruitment_System_email_source,
                                                          Destination={'ToAddresses': [email]},
                                                          Message={'Subject': {'Data': msg, 'Charset': 'utf-8'},
                                                                   'Body': {'Html': {'Data': msg, 'Charset': 'utf-8'}}})
...
def schedule_interview_action():
    ...
    if email in emails['Identities']:
        msg = 'Hello ' + name + ' Your Job Interview has been Scheduled on '+str(date_time)
        ses_client_Employee_Recruitment_System.send_email(Source=Employee_Recruitment_System_email_source,
                                                          Destination={'ToAddresses': [email]},
                                                          Message={'Subject': {'Data': msg, 'Charset': 'utf-8'},
                                                                   'Body': {'Html': {'Data': msg, 'Charset': 'utf-8'}}})
